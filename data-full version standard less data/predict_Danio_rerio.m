...
init_cond = [V0; E_0; 0; 0; 1];
F = f_ValKwa2022;
[~, VEHRsMG] = ode45(@ode_VEHRsM, [0; data.tL_ValKwa2022(:,1)], init_cond, [], par, F, TC_26_72);
V = VEHRsMG(2:end, 1); E = VEHRsMG(2:end, 2); E_H = VEHRsMG(2:end, 3); E_R = VEHRsMG(2:end, 4); s_M = VEHRsMG(2:end, 5); 
[p_A, ~, ~, ~, ~, ~] = compute_powers(V, E, E_H, s_M, TC_26_72, F, par);

L = V.^(1/3);
W = 1 * V + (E + E_R .* countERForWeight) * w_E / mu_E / d_E;
...
init_cond = [V0; E_0; 0; 0; 1];
F = f_YangYama2019;
[~, VEHRsMG] = ode45(@ode_VEHRsM, [0; data.tL_YangYama2019(:,1)], init_cond, [], par, F, TC_28);
V = VEHRsMG(2:end, 1); E = VEHRsMG(2:end, 2); E_H = VEHRsMG(2:end, 3); E_R = VEHRsMG(2:end, 4); s_M = VEHRsMG(2:end, 5); 

[p_A, ~, p_S, p_G, p_J, p_R] = compute_powers(V, E, E_H, s_M, TC_28, F, par);
p_D = compute_dissipation_power(p_S, p_J, p_R, E_H, E_Hp, kap_R);

eta_M = -inv(n_M) * n_O * eta_O;
J_O = [p_A, p_D, p_G] * eta_M(3, :)'; % mol/d
J_O = -J_O * 32 * 1e6 / 24; % mug/hr
W = 1 * V + (E + E_R .* countERForWeight) * w_E / mu_E / d_E; % g

prdData.tL_YangYama2019 = V.^(1/3) / del_Ms; % cm
...
[~, VEHRsMG] = ode45(@ode_VEHRsM, t_sort, init_cond, [], par, F, TC);
V = VEHRsMG(:, 1); L = V.^(1/3);
...
[~, VEHRsMG] = ode45(@ode_VEHRsM, [0; tL_BagaPels2001(:,1)], init_cond, [], par, F, TC);
V = VEHRsMG(2:end, 1); E = VEHRsMG(2:end, 2); E_R = VEHRsMG(2:end, 4); 

prdData.tWw_BagaPels2001  = 1 * V + w_E / mu_E / d_E * (E + E_R .* countERForWeight); % g, wet weight
prdData.tWd_BagaPels2001  = d_V * V + w_E / mu_E * (E + E_R .* countERForWeight); % g, dry weight
...
init_cond = [V_init; E_init; E_Hp; 0; s_M_F];
[~, VEHRsMG] = ode45(@ode_VEHRsM, [0; data.tN(:,1)], init_cond, [], par, F, TC);
V = VEHRsMG(:, 1); E = VEHRsMG(:, 2); E_R = VEHRsMG(:, 4); 

prdData.tN = E_R(2:end) / E_0;
prdData.tL1 = V([1 end]).^(1/3) / del_Ms;
prdData.Wwt = 1 * V(end) + w_E / mu_E / d_E * (E(end) + E_R(end) .* countERForWeight);

end

%% SUBFUNCTIONS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

function [p_A, p_C, p_S, p_G, p_J, p_R] = compute_powers(V, E, E_H, s_M, TC, f, p)
...
function p_D = compute_dissipation_power(p_S, p_J, p_R, E_H, E_Hp, kap_R)
p_D = p_S + p_J + p_R .* (E_H < E_Hp) + (1 - kap_R) * p_R .* (E_H >= E_Hp);
end
...
[p_A, p_C, p_S, p_G, p_J, p_R] = compute_powers(V, E, E_H, s_M, TC, f, p);

% State changes
dE = p_A - p_C;
dV = p_G / E_G;
dE_H = p_R * (E_H < E_Hp);
dE_R = kap_R * p_R * (E_H >= E_Hp);
